...
class AuthenticationMiddleware {
    ...
    authentication = async (req, res, next) => {
        ...
        if (decoded?.jti) {
            const revokedToken = await this._redisService.getMethod(`revokedTokens:${decoded.userId}:${decoded.jti}`);
            if (revokedToken) {
        ...
        const revokedToken = await this._redisService.getMethod(`revokedTokens:${user._id.toString()}:${decoded.jti}`);
        if (revokedToken) {
            throw new global_error_handling_1.AppError("Unauthorized access, token is revoked", 401);
        }
        const request = req;
